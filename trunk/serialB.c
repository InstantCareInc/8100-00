...
//
    ...
    U1CTL_bit.SWRST = 0;      // Clear SWRST, enable UART
    IE2_bit.URXIE1 = 1;       // Allow receive interrupts immediately
}
...
__interrupt void serialB_tx_isr(void)
{
    ...
    {
        IE2_bit.UTXIE1 = 0;           // No - disable the transmit interrupt
    }
}
...
int serialB_read(unsigned char *data)            // Check serial port for pending command character
{
    ...
    {
        IE2_bit.URXIE1 = 0;            // Protect our ring buffer updates from the Rx ISR.
        ...
        *data = *serialB_InGet++;
        ...
        IE2_bit.URXIE1 = 1;                // Allow receive interrupts again
        return (TRUE);
    }
...
int serialB_write(unsigned char data)
{
    ...
    {
        IE2_bit.UTXIE1 = 0;            // Protect our ring buffer updates from the Tx ISR.
        *serialB_OutPut = data;
        serialB_OutPut++;
        // Check if past end of ring
        ...
        IE2_bit.UTXIE1 = 1;            // Allow transmit interrupts
        return (TRUE);
    }
    else {
