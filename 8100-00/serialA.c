...
//
    ...
    U0CTL &= ~SWRST;	// Clear SWRST, enable UART
...
static int serialA_output_byte(void)
{
    IFG1 &= ~UTXIFG0;           // Clear the transmitter interrupt
...
__interrupt void serialA_tx_isr(void)
{
    ...
    if ((serialA_ignoreCTS_flag == TRUE) || (CPU_IO_MS_CTS == 0))
    {
        if (serialA_output_byte() == 0) {
            IE1 &= ~UTXIE0;         // Didn't start, disable the transmitter interrupt
        }
    }
    else {
...
void serialA_restart_output(void)
{
    if ((serialA_ignoreCTS_flag == TRUE) || (CPU_IO_MS_CTS == 0))
    {
        if (serialA_OutCount)
        {
            ...
            {
                IE1 &= ~UTXIE0;                     // Disable the interrupt first, so we can't collide with the ISR
                if (serialA_output_byte()) {  // Yes - start it.
                    IE1 |= UTXIE0;       // If there was something to output, Enable the transmitter interrupt
                }
            }
        }
    }
...
int serialA_read(unsigned char *inputA_data)	        // Check serial port for pending command character
{
    ...
    {
        IE1 &= ~URXIE0;         // Protect our ring buffer updates from the Rx ISR.
        ...
        *inputA_data = *serialA_InGet++;
        ...
        IE1 |= URXIE0;         // Allow receive interrupts again
...
int serialA_write(unsigned char data)
{
    ...
    {
        IE1 &= ~UTXIE0;               // Protect our ring buffer updates from the Tx ISR.
        *serialA_OutPut++ = data;
        ...
        {
            IFG1 &= ~UTXIFG0;         // Clear the transmitter interrupt flag
            if (serialA_output_byte()) { // Yes - start it.
            IE1 |= UTXIE0;      // If there was something to output, Enable the transmitter interrupt
            }
        }
        else {
            IE1 |= UTXIE0;           // Allow transmit interrupts
        }
        return (TRUE);
    }
